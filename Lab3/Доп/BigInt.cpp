...
namespace Prog3b {
	...
	bool BigInt::CompareAbs(const BigInt& t) const { // сравнение модулей 
	...
	BigInt operator +(const BigInt fir, const BigInt sec) { //сумма
		int dop = 0;
	...
	bool BigInt::operator >(const BigInt& second) { // сравнение
	...
	BigInt& BigInt::operator <<=(int pr) { // увеличить ан 10
		if (n == 1 && Int[1] == 0) {
			n = 1;
			return *this;
		}
		Int[0] = Int[0];
		if (/*Int[SZ - pr] != 0*/SZ - n < pr)
			throw std::runtime_error("Overflow");
	...
	BigInt BigInt::operator >>=(int pr) { //уменьшение на 10
		if (!(n - pr > 0)) {
			for (int i = 0; i <= n; i++) {
				Int[i] = 0;
			}
	...
	std::istream& operator >>(std::istream& s, BigInt& t) { // вывод 
		try {
			char ptr[t.SZ] = "";
			int len = 0;
			s >> ptr;
	...
	std::ostream& operator <<(std::ostream& s, const BigInt& t) {
		if (t.Int[0] == 1)
			s << "-";
		if (t.n == 1 && t.Int[1] == 0) { s << 0; return s; }
	...
	BigInt::operator int() const { //переврит в число 
		int i = 0;
		int pow = 1;
		if (n > 50)
			throw - 1;
