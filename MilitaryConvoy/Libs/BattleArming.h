...
namespace MC {
	class BatArm {
		...
		int price;
	public:
		BatArm(std::string type = "", int damage = 0, int ratefire = 0, int range = 0, int maxammo = 0, int ammo = 0, int price = 0) :type(type),
			damage(damage), ratefire(ratefire), range(range), maxammo(maxammo), ammo(maxammo), price(price) {};
